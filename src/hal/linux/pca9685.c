...
pca9685_write(int fd, uint8_t channel, uint16_t value)
{
    ...
    if (ret < 0) {
        ...
        sched_try_shutdown("Unable to update PCA9685 value");
    }
}
    ...
    for (i=0; i<devices_count; i++)
        if (devices[i].bus == bus && devices[i].addr == addr) {
            if (cycle_ticks != devices[i].cycle_ticks)
                sched_shutdown("All PCA9685 channels must have the same cycle_ticks");
            return devices[i].fd;
        }

    // Setup new I2C device
    if (devices_count >= ARRAY_SIZE(devices))
        sched_shutdown("Too many i2c devices");
    char fname[256];
...
fail:
    if (fd >= 0)
        close(fd);
    sched_shutdown("Unable to open and init PCA9685 device");
}
...
{
    sched_shutdown("Missed scheduling of next pca9685 event");
}
...
command_config_pca9685(uint32_t *args)
{
    ...
    if (channel > CHANNEL_MAX || value > VALUE_MAX || default_value > VALUE_MAX)
        sched_shutdown("Invalid pca9685 channel or value");
...
command_schedule_pca9685_out(uint32_t *args)
{
    ...
    if (p->value > VALUE_MAX)
        sched_shutdown("Invalid pca9685 value");
