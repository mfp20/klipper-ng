...
spi_setup(uint32_t bus, uint8_t mode, uint32_t rate)
{
    ...
    if (ret < 0) {
        ...
        sched_shutdown("Unable to set SPI speed");
    }
             ...
             , uint8_t len, uint8_t *data)
{
    ...
    if (receive_data) {
        ...
        if (ret < 0) {
            ...
            sched_try_shutdown("Unable to issue spi ioctl");
        }
    } else {
        int ret = write(config.fd, data, len);
        if (ret < 0) {
            report_errno("write spi", ret);
            sched_try_shutdown("Unable to write to spi");
        }
    }
}
