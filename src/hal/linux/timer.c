...
#include "generic_irq.h" // irqstatus_t
#include "console.h" // console_sleep
...
static inline uint32_t timespec_to_time(struct timespec ts) {
...
static inline struct timespec timespec_from_time(uint32_t time) {
...
static inline struct timespec timespec_add(struct timespec ts, long ns) {
    ts.tv_nsec += ns;
    if (ts.tv_nsec >= NSECS) {
        ts.tv_sec++;
        ts.tv_nsec -= NSECS;
